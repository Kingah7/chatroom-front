...
const logined = (to, from, next) => {
    if (to.path == "/login" || to.path == "/") {
        // 已经登录
        next(false);
        Vue.prototype.$message.warning("您已登录，无需重复cao'z")
    }
    else next();
}

const unlogin = (to, from, next) => {
    // 未登录
    if (to.path != "/login")
...
router.beforeEach((to, from, next) => {
    ...
    if (store.state.userName == "") {
        reqLogin().then(resp => {
            // 如果已经登录
            if (resp.code == 200) logined(to, from, next);
            else unlogin(to, from, next);
        })
    }
    else logined(to, from, next);
