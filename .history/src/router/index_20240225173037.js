import Vue from "vue";
import VueRouter from "vue-router";

import Login from "@/pages/LoginPage";
import ChatRoom from "@/pages/chat";
import store from "@/store"
import { reqLogin } from "@/api";

Vue.use(VueRouter);

const originPush = VueRouter.prototype.push;
VueRouter.prototype.push = function (location, success, error) {
    if (success && error) {
        originPush.call(this, location, success, error);
    } else {
        originPush.call(this, location, () => { }, () => { })
    }
}


const router = new VueRouter({
    routes: [
        {
            path: "/",
            redirect: "/login"
        },
        {
            path: "/login",
            component: Login
        },
        {
            path: "/chat",
            component: ChatRoom
        }
    ]
})

// 全局守卫的逻辑
const routerLogical = (to, from, next) => {
    if (store.state.userName != "" && store.state.userName != null) {
        if (to.path == "/login") {
            // 已经登录
            next(false);
            Vue.prototype.$message.warning("您已登录，无需再次")
        }
        else next();
    }
    else {
        // 未登录
        if (to.path != "/login")
            Vue.prototype.$message.error("您还未登录，请先登录!");
        
        if (to.path == "/login") next();
        else next("/login");
    }
}


router.beforeEach((to, from, next) => {
    // 如果仓库被清空那就发一次请求查验是否已经登录
    if (store.state.userName == "") reqLogin().then((resp) => {
        if (resp.code == 200) store.commit("SETUSERNAME", resp.data);
        routerLogical(to, from, next)
    });
    else routerLogical(to, from, next)

})

export default router;